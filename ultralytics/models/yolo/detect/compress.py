...
from ultralytics.nn.modules import Detect, v10Detect, RepConv, PSA, AIFI
...
# 没有购买yolov11项目需要注释以下
from ultralytics.nn.extra_modules import Detect_Efficient, Detect_DyHead_Prune, Detect_LSCD, Detect_RSCD, Detect_TADDH, Detect_SEAM, Detect_LQE
from ultralytics.nn.extra_modules.block import Faster_Block, Fusion, IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, SimFusion_4in, AdvPoolFusion, \
    PyramidPoolAgg, RepVGGBlock, RepConvN, Star_Block, Faster_Block_CGLU, MutilScaleEdgeInformationEnhance, CSPOmniKernel, SPDConv, Bottleneck_EMSCP, Zoom_cat, HGStem, Rep_HGBlock, \
    DWConv, FeaturePyramidSharedConv, DynamicScalSeq, asf_attention_model, ScalSeq, MutilScaleEdgeInformationSelect, SBA
from ultralytics.nn.extra_modules.attention import CoordAtt
...
def get_pruner(opt, model, example_inputs):
    ...
    # for yolo11-C3k2_EMSCP.yaml
    # for k, m in model.named_modules():
    #     if isinstance(m, Detect_SEAM):
    #         ignored_layers.append(m)
    #     if isinstance(m, PSABlock):
    #         ignored_layers.append(m.attn)
        # if isinstance(m, asf_attention_model):
        #     ignored_layers.append(m)
        # if isinstance(m, ScalSeq):
        #     ignored_layers.append(m)
        # if isinstance(m, CoordAtt):
        #     ignored_layers.append(m)
        # if isinstance(m, Bottleneck_EMSCP):
        #     ignored_layers.append(m)
        # if isinstance(m, Zoom_cat):
        #     ignored_layers.append(m)
        # if isinstance(m, FeaturePyramidSharedConv):
        #     ignored_layers.append(m)
        # ignored_layers.append(model.model[32])
        
        

    # for yolo11-C3k2-Faster-CGLU-RSCD.yaml
    # for k, m in model.named_modules():
    #     if isinstance(m, Detect_RSCD):
    #         ignored_layers.append(m)
    #     if isinstance(m, PSABlock):
    #         ignored_layers.append(m.attn)
    #     if isinstance(m, Faster_Block_CGLU):
    #         ignored_layers.append(m.mlp.fc1)
    #         ignored_layers.append(m.mlp.fc2)
    
    # for yolo11-FDPN-TADDH.yaml
    # for k, m in model.named_modules():
    #     if isinstance(m, Detect_TADDH):
    #         ignored_layers.append(m)
    #     if isinstance(m, PSABlock):
    #         ignored_layers.append(m.attn)
    # ignored_layers.append(model.model[4].cv2)
    # ignored_layers.append(model.model[17].cv2)
    
    # for yolo11-C3k2-MutilScaleEdgeInformationEnhance.yaml
    # for k, m in model.named_modules():
    #     if isinstance(m, Detect):
    #         ignored_layers.append(m.cv2[0][2])
    #         ignored_layers.append(m.cv2[1][2])
    ...
    #     if isinstance(m, PSABlock):
    #         ignored_layers.append(m.attn)
    #     if isinstance(m, MutilScaleEdgeInformationEnhance):
    #         ignored_layers.append(m)
    
    # for yolo11n-SOEP.yaml
    # for k, m in model.named_modules():
    #     if isinstance(m, Detect):
    #         ignored_layers.append(m.cv2[0][2])
    #         ignored_layers.append(m.cv2[1][2])
    ...
    #         ignored_layers.append(m)
    # ignored_layers.append(model.model[18])
    # ignored_layers.append(model.model[19])
    # ignored_layers.append(model.model[21])
    # ignored_layers.append(model.model[22])
    # ignored_layers.append(model.model[24])
...
class DetectionCompressor(BaseTrainer):
    ...
    def save_model(self):
        ...
        if self.best_sl[f'{self.sparsity_ratio:.3f}'] == self.fitness:
            torch.save({'model': deepcopy(de_parallel(self.model)).half(),
                        'ema': deepcopy(self.ema.ema).half(),}, self.wdir / 'best_sl_{:.3f}.pt'.format(self.sparsity_ratio), pickle_module=pickle)
    ...
    def model_prune(self, imp, prune, example_inputs):
        ...
        with HiddenPrints():
            try:
                ...
                ori_flops, ori_params = profile(base_model, [example_inputs])
        ...
        while speed_up < self.args.speed_up:
            ...
            with HiddenPrints():
                try:
                    ...
                    prune_flops, prune_params = profile(self.model, [example_inputs])
    ...
    def sparsity_learning(self, ckpt, world_size, prune):
        ...
        self.batch_size = self.batch_size // 4  # 内存比较小，可以适当增加
    ...
    def compress(self):
        ...
        torch.save({'model':deepcopy(self.model).half(), 'ema':None}, self.wdir / 'model_c2f_v2.pt', pickle_module=pickle)
