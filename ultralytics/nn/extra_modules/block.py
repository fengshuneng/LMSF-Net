...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'C3k2_RFAConv', 'C3k2_RFCBAMConv', 'C3k2_RFCAConv', 'Ghost_HGBlock', 'Rep_HGBlock', 'RS_HGBlock',
           'C3k2_FocusedLinearAttention', 'C3k2_MLCA', 'AKConv', 'C3k2_AKConv',
           'C3k2_UniRepLKNetBlock', 'C3k2_DRB', 'C3k2_DWR_DRB', 'Zoom_cat', 'ScalSeq', 'DynamicScalSeq', 'Add', 'CSP_EDLAN', 'asf_attention_model', 'ASF_LearnAtt', ''
           ...
           'CSP_MSCB_SC', 'EUCB_SC', 'C3k2_FAT', 'A2C2f_FMFFN', 'A2C2f_FMFFN_DYT', 'C3k2_LEGM', 'C3k2_RCB', 'LoGStem', 'C3k2_LFEM', 'C3k2_SBSM', 'A2C2f_SEFN', 'A2C2f_Mona', 'C3k2_LSBlock', 'C3k2_MambaOut_LSConv', 'A2C2f_DFFN_DYT_Mona'
           ]
...
class ASF_LearnAtt(nn.Module):
  
    def __init__(self, ch=256):
        super().__init__()
        self.ch = ch
        # reuse existing attention modules
        self.channel_att1 = asf_channel_att(ch)
        self.channel_att2 = asf_channel_att(ch)
        self.local_att = asf_local_att(ch)

        # fuse conv: take concat(p1,p2) -> 1x1 conv -> sigmoid -> per-channel gate (B, ch, 1, 1)
        self.fuse_conv = nn.Conv2d(ch * 2, ch, kernel_size=1, bias=True)

        # lazy 1x1 projections if input channels differ from target ch
        self.proj1 = None
        self.proj2 = None

    def _ensure_proj(self, in_c1, in_c2, device):
        """
        Ensure proj1/proj2 exist to map in_c1/in_c2 -> self.ch if necessary.
        Create them lazily on the right device.
        """
        if in_c1 != self.ch:
            self.proj1 = nn.Conv2d(in_c1, self.ch, kernel_size=1, bias=False).to(device)
        else:
            self.proj1 = None
        if in_c2 != self.ch:
            self.proj2 = nn.Conv2d(in_c2, self.ch, kernel_size=1, bias=False).to(device)
        else:
            self.proj2 = None

    def forward(self, x):
        # x expected: iterable [input1, input2]
        input1, input2 = x[0], x[1]

        # 1) per-input channel attention (keeps original spatial sizes)
        a1 = self.channel_att1(input1)  # (B, C1, H1, W1)
        a2 = self.channel_att2(input2)  # (B, C2, H2, W2)

        # 2) make channels equal to self.ch using lazy 1x1 convs if needed
        c1, c2 = a1.shape[1], a2.shape[1]
        if (c1 != self.ch) or (c2 != self.ch):
            self._ensure_proj(c1, c2, a1.device)
            if self.proj1 is not None:
                a1 = self.proj1(a1)
            if self.proj2 is not None:
                a2 = self.proj2(a2)

        # 3) align spatial sizes by upsampling the smaller to the larger (nearest)
        h1, w1 = a1.shape[2], a1.shape[3]
        h2, w2 = a2.shape[2], a2.shape[3]
        if (h1, w1) != (h2, w2):
            target_h = max(h1, h2)
            target_w = max(w1, w2)
            if (h1, w1) != (target_h, target_w):
                a1 = F.interpolate(a1, size=(target_h, target_w), mode='nearest')
            if (h2, w2) != (target_h, target_w):
                a2 = F.interpolate(a2, size=(target_h, target_w), mode='nearest')

        # 4) core behavior: elementwise sum (preserve original semantics)
        fused_sum = a1 + a2  # (B, ch, H, W)

        # 5) small per-channel modulation (lightweight innovation)
        #    compute pooled descriptors (B, ch, 1, 1)
        p1 = torch.mean(a1, dim=(2, 3), keepdim=True)
        p2 = torch.mean(a2, dim=(2, 3), keepdim=True)
        pooled = torch.cat([p1, p2], dim=1)         # (B, 2ch, 1, 1)
        logits = self.fuse_conv(pooled)             # (B, ch, 1, 1)
        gate = torch.sigmoid(logits)                # (B, ch, 1, 1) in (0,1)

        # scale around 1.0, small modulation: e.g., 0.5 + gate in [0.5,1.5]
        scale = 0.5 + gate

        fused = fused_sum * scale  # elementwise, preserves sum semantics but modulates per-channel

        # 6) final spatial refinement
        out = self.local_att(fused)
        return out
...
class C3k2_TSSA(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_TSSA(self.c, self.c, n, size, shortcut, g) if c3k else MetaFormerBlock(
...
######################################## CVPR2025-LSNet end ########################################
class RS_HGBlock(nn.Module):
    """
    Minimal, low-risk enhancement of Rep_HGBlock:
      - keep original sequential block logic
      - add lightweight SE (squeeze-and-excite) after ec
      - ensure SE initialized so initial scale == 1.0 (no change at init)
    Constructor signature kept compatible with original; added optional se_ratio parameter.
    """
    def __init__(self, c1, cm, c2, k=3, n=6, lightconv=False, shortcut=False, act=True, se_ratio=16):
        super().__init__()
        block = RepLightConv if lightconv else Conv
        # original block list (same as Rep_HGBlock)
        self.m = nn.ModuleList(block(c1 if i == 0 else cm, cm, k=k, act=act) for i in range(n))
        self.sc = Conv(c1 + n * cm, c2 // 2, 1, 1, act=act)  # squeeze conv
        self.ec = Conv(c2 // 2, c2, 1, 1, act=act)  # excitation conv
        self.add = shortcut and c1 == c2

        # --- lightweight SE (squeeze-and-excite) ---
        # If se_ratio <= 1 then SE disabled for safety
        self.se_ratio = int(se_ratio) if se_ratio is not None else 0
        if self.se_ratio and self.se_ratio > 1:
            se_hidden = max(1, c2 // self.se_ratio)
            self.se_fc1 = nn.Conv2d(c2, se_hidden, kernel_size=1, bias=True)
            self.se_act = nn.ReLU(inplace=True)
            self.se_fc2 = nn.Conv2d(se_hidden, c2, kernel_size=1, bias=True)
            # IMPORTANT: initialize se_fc2 weights to 0 so initial logits == 0,
            # sigmoid(0) == 0.5 -> scale = sigmoid*2.0 == 1.0 (no initial change).
            nn.init.constant_(self.se_fc2.weight, 0.0)
            nn.init.constant_(self.se_fc2.bias, 0.0)
        else:
            self.se_fc1 = None
            self.se_fc2 = None

    def forward(self, x):
        # same forward as original but with SE after ec
        y_list = [x]
        # original sequential accumulation
        for m in self.m:
            y_list.append(m(y_list[-1]))
        y = self.ec(self.sc(torch.cat(y_list, 1)))

        # SE recalibration (if enabled)
        if self.se_fc1 is not None:
            s = F.adaptive_avg_pool2d(y, 1)             # (B, c2, 1, 1)
            s = self.se_fc1(s)
            s = self.se_act(s)
            s = self.se_fc2(s)                         # logits
            s = torch.sigmoid(s) * 2.0                 # scale in (0, 2), init ~1.0
            y = y * s
